...
# # Define paths
# ARTIFACTS_PATH = "D:\MLOPS_PRACTICAL_1BM22AI065\PRACTICAL\Artifacts"
# DATA_OUTPUT_PATH = "D:\MLOPS_PRACTICAL_1BM22AI065\PRACTICAL\Data\output"
...
if page == "Single Prediction":
    ...
    if st.button("Predict Risk Category"):
        ...
        if prediction:
            ...
            # Save single prediction input and result
            result_df = pd.DataFrame([input_data])
            result_df['Predicted Risk Category'] = prediction
            single_output_file = os.path.join(DATA_OUTPUT_PATH, "single_predictions_log.csv")
            
            if os.path.exists(single_output_file):
                result_df.to_csv(single_output_file, mode='a', header=False, index=False)
            else:
                result_df.to_csv(single_output_file, index=False)
            
            st.info(f"Prediction logged at: {single_output_file}")
